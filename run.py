#!/usr/bin/env python3
# -*- coding:utf-8 -*-
"""Main entrypoint to run racecar."""
...
if config.fpv:
    ...
    server = Process(
        target=fpv.run,
        args=data_sh,
        kwargs={"host": "localhost", "port": config.port, "threaded": True},
    )
    server.start()
...
if config.HAVE_NN:
    ...
    model = train_pytorch.NeuralNetwork(len(config.ultrasonics_list), 2, config.hidden_dim, config.num_hidden_layers)
    ## 保存したモデルをロード
    ...
    while True:
        ...
        if recording:
            d_stack = np.vstack(
                (
                    d_stack,
                    np.insert(d, 0, [ts, steer_pwm_duty, throttle_pwm_duty]),
                )
            )
        ...
        if config.plotter:
            ...
            print(
                "Rec:{0}, Mode:{1}, RunTime:{2:>5}, Str:{3:>4}, Thr:{4:>4}, Uls:[ {5}]".format(
                    recording, mode, ts_run, steer_pwm_duty, throttle_pwm_duty, message
                )
            )  # ,end=' , '
